...
const Food = () => {
	...
	const getFood = async () => {
		...
		const url = "https://gtg.seabird.digital/api/schedule/food" +
			`?restaurant=${ ["ra", "tp", "rh"][selectedIndex] }`;
		console.log(url);
				...
				<View style={ { marginTop: -16, marginHorizontal: -24 } }>
					...
					<View style={ { paddingHorizontal: 24 } }>
						...
						{ loading ? (
							...
							(food?.[days % 7]?.dishes ? food[days % 7].dishes.map((d) => {
								return (
									<Card style={ { marginVertical: 4 } }>
										<View style={ { flexDirection: "row", justifyContent: "space-between" } }>
											<Text category={ "s1" }
											      style={ { width: "75%" } }>{ d.name ?? "" }</Text>
											<Text category={ "p1" }>{ d.price ? `${ d.price }kr` : "" }</Text>
										</View>
										<Text category={ "s1" }
										      appearance={ "hint" }>{ d.type ?? "" } { d.allergies ? `(${ d.allergies })` : "" }</Text>
									</Card>
								);
							}) : (
								<Card>
									<Text>{ error ? "Sum ting wong" : "Menyn för idag inte tillgänglig" }</Text>
								</Card>
							)) }
					</View>
				</View>
