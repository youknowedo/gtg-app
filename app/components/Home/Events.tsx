...
const Events = ({ screenProps }: EventsProps) => {
		...
		const getEvents = async () => {
			...
			const url = "https://gtg.seabird.digital/api/schedule/exams" +
				`?class=${ store.getState().classes.classes?.[store.getState().classes.selectedIndex].groupName }` +
				`&week=${ weekNumber }`;
			console.log((url));
		...
		return (
			<>
				...
				<View>
					...
					) : (events && events.length > 0 ? events.map((day) => {
						return day.exams.map((e) => {
							return (
								<Card style={ { marginVertical: 4 } }>
									<View style={ { flexDirection: "row", justifyContent: "space-between" } }>
										<Text category={ "s1" }>{ e.name }</Text>
										<Text category={ "p1" }>{ new Date(day.date).toLocaleString("sv-SE", {
											weekday: "long"
										}) }</Text>
									</View>
									<Text category={ "s1" } appearance={ "hint" }>{ e.teacher }</Text>
								</Card>
							);
						});
					}) : (
						<Card>
							<Text>{ error ? "Sum ting wong" : "Inga händelser idag" }</Text>
						</Card>
					)) }
				</View>
			</>
